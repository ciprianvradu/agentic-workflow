...
This document describes how the agentic-workflow system works across all five AI host platforms (Claude Code, GitHub Copilot, Gemini CLI, OpenCode, Devin) and all operating system targets (Linux, WSL, macOS, native Windows).
...
| OS Environment | Claude Code | Copilot CLI | Gemini CLI | OpenCode | Devin |
|---|---|---|---|---|---|
| Linux (native) | Yes | Yes | Yes | Yes | Yes |
| macOS | Yes | Yes | Yes | Yes | Yes |
| WSL (file on ext4) | Yes | Yes | Yes | Yes | Yes |
| WSL (file on /mnt/) | Yes (slow) | Yes (slow) | Yes (slow) | Yes (slow) | Yes (slow) |
| Native Windows | Partial | Partial | Partial | Yes | Partial |
