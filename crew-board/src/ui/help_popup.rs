...
pub fn draw(frame: &mut Frame, app: &App) {
    ...
    let lines = vec![
        ...
        key_line("F5", "Force refresh (non-Terminals) / Prev terminal (Terminals Normal)", key_style),
        key_line("F6", "Documents (Tasks) / Next terminal (Terminals Normal) / Cleanup (others)", key_style),
        ...
        Line::from(""),
        Line::from(Span::styled("Tasks View", bold)),
        Line::from(""),
        key_line("f", "Cycle task filter: All / Active / Active+Recent (left pane)", key_style),
        Line::from(""),
        ...
        key_line("F5", "Previous running terminal (skip exited)", key_style),
        key_line("F6", "Next running terminal (skip exited)", key_style),
        key_line("F12", "Focus terminal (all keys \u{2192} PTY)", key_style),
        key_line("Enter", "Relaunch exited terminal", key_style),
        key_line("Delete", "Dismiss exited terminal", key_style),
        ...
        Line::from(""),
        Line::from(Span::styled("  Tile Navigation (tiled/stacked layouts):", dim)),
        key_line("Alt+Left/Right", "Move focus between horizontal tiles", key_style),
        key_line("Alt+Up/Down", "Move focus between vertical tiles/rows", key_style),
        key_line("Click", "Click on a tile to focus it (no drag)", key_style),
        Line::from(""),
